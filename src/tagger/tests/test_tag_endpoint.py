...
class TestTagEndpoint(unittest.TestCase):
    ...
    def test_missing_mode(self):
        response = self.client.get(
            "/tag?sentence=How%20John%20is%20doing%20in%20Japan%3F"
        )
        data = response.get_json()
    ...
    def test_wrong_mode(self):
        response = self.client.get(
            "/tag?mode=wrong_mode&sentence=How%20John%20is%20doing%20in%20Japan%3F"
        )
        data = response.get_json()
        self.assertEqual(response.status_code, 400)
    ...
    def test_mode_as_number(self):
        response = self.client.get(
            "/tag?mode=123&sentence=How%20John%20is%20doing%20in%20Japan%3F"
        )
        data = response.get_json()
        self.assertEqual(response.status_code, 400)
        self.assertEqual(data, {"error": messages.INVALID_MODE_ERROR})

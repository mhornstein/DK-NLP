...
from ..services.db_util import get_collection
...
@api_blueprint.route("/add_entry", methods=["POST"])
def add_entry():
    """
...
@api_blueprint.route("/fetch_entries", methods=["GET"])
def fetch_entries():
    """
    ...
    * end_of_history - flag to indicate whether the oldest entry in the collection has been fetched, signifying the end of the available history.
    ...
    try:
        ...
        result = {
            "entries": [
                {"_id": str(entry["_id"]), "date": entry["date"], "tagged_sentence": entry["tagged_sentence"]}
        ...
        if oldest_entry is None:  # The collection is empty
            ...
            oldest_entry["_id"] == entries[-1]["_id"]
        ):  # the collection is not empty and the last entity was fetched
            result["end_of_history"] = True
        else:
